...
impl UPBitSocket {
    ...
    pub async fn refresh_recommended_coins(&mut self) -> Result<(f64, f64, f64), UPBitError> {
        let prev_delta = self.previous_bei_delta;
        let (bei, bei_delta, bersi) = match self.btc_eth_indicator().await {
            Ok((a, b, c)) => (a, b, c),
            Err(e) => return Err(e),
        };
                ...
                for ticker in heavy_tickers {
                   let df = match self.get_recent_market_data(ticker.as_str(), 200).await {
                    ...
                    let df_minus_std = df.clone().lazy().select([
                        (col("trade_price")-col("trade_price").std(0)).alias("delta")
                    ]).collect().unwrap();
    ...
    // 초 단위 짧은 시간 반복 호출 권장
    pub async fn buy_decision(self) {
        for (ticker, mut coin) in self.recommended_coins {
            let prev_rsi = coin.previous_rsi;
            let current_rsi = coin.get_rsi().await.unwrap();
            coin.previous_rsi = current_rsi;
